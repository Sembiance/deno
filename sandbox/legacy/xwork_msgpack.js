...
import {TextLineStream, msgpackEncode, msgpackDecode, base64Encode, base64Decode} from "std";

const xwork = {};

// sigh. msgpack implementation in deno doesn't handle 'undefined' so we mimick how JSON.stringify handles this by converting undefined in array. msgpackr from node has an option to support this behavior but deno has no options to control behavior
function filterUndefined(v)
{
	if(v===undefined)
		return null;
	...
	if(Array.isArray(v))
	{
		v.mapInPlace(filterUndefined);
	}
	else if(Object.isObject(v))
	{
		const keysToDelete = [];
		Object.mapInPlace(v, (subKey, subVal) =>
		{
			if(subVal===undefined)
			{
				keysToDelete.push(subKey);
				return [subKey, subVal];
			}
			return [subKey, filterUndefined(subVal)];
		});
...
// ran into an issue where JSON.stringify would fail if the object was too large (discmaster item #1884) so we use msgpack instead now. Since we are using binary now, should probably ditch the base64encoding and redo sockUtil to support <len><data> but meh
const encodeMessagePacket = o => base64Encode(msgpackEncode(filterUndefined(o)));
const decodeMessagePacket = (msg, fallback) =>
{
	try { return msgpackDecode(base64Decode(msg)); }
	catch {	return fallback; }
};

/** THESE ARE ALL CALLED BY THE INDIVIDUAL WORKERS, DO NOT CALL THESE AS THE PARENT */
xwork.arg = async function arg() { return decodeMessagePacket(await sockUtil.sendReceiveLine({transport : "unix", path : Deno.env.get("XWORK_SOCK_PATH")}, encodeMessagePacket({op : "arg"})), []); };
xwork.done = async function done(msg) { return await sockUtil.sendLine({transport : "unix", path : Deno.env.get("XWORK_SOCK_PATH")}, encodeMessagePacket({op : "done", msg})); };
...
xwork.openConnection = async function openConnection()
{
	...
	const receiveMessages = async function()
	{
		try
		{
			for await(const line of workerConnection.readable.pipeThrough(new TextDecoderStream()).pipeThrough(new TextLineStream()))
				workerMessages.push(decodeMessagePacket(line, {}));
		}
		catch {}
	...
	receiveMessages();
	await new Blob([textEncoder.encode(encodeMessagePacket({op : "ready"}) + "\n")]).stream().pipeTo(workerConnection.writable, {preventClose : true});
};
...
xwork.send = async function send(msg)
{
	...
	await (new Blob([textEncoder.encode(encodeMessagePacket({op : "msg", msg}) + "\n")]).stream()).pipeTo(workerConnection.writable, {preventClose : true});
};
...
xwork.run = async function run(fun, arg, {timeout, detached, imports={}, recvcb, exitcb, hideOutput, xlog, stderrcb, runEnv, runArgs=[]}={})
{
	...
	{
		const {op, msg} = decodeMessagePacket(line, {});
		if(!op)
			return;
		...
		if(op==="arg")
			await (new Blob([textEncoder.encode(encodeMessagePacket(arg) + "\n")]).stream()).pipeTo(conn.writable, {preventClose : true});
	...
	return {
		...
		send  : async msg =>
		{
			...
			await (new Blob([textEncoder.encode(encodeMessagePacket(msg) + "\n")]).stream()).pipeTo(workerMsgConn.writable, {preventClose : true});
		}
	};
};
